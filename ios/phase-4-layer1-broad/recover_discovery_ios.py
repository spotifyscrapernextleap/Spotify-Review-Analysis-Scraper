"""Phase 4 -> Phase 5 bridge (iOS) — DISCOVERY RECALL RECOVERY (gpt-oss-120b).

iOS analogue of phases/phase-4-layer1-broad/recover_discovery.py (D12). The Android
recall probe found the 8B's missed discovery reviews hide almost entirely in the `ux`
and `updates` piles; pricing/tech/etc. are clean. We run the 120B discovery-detector
over the FULL iOS ux + updates piles (a census, not a sample) to (a) get reliable
per-pile miss rates and (b) RECOVER the missed discovery reviews and fold them into the
iOS Phase-5 deep-coding pool.

The 120B pool is an INDEPENDENT Groq quota from the 8B Layer-1 pool, so this does not
compete with the iOS Layer-1 classification budget.

Reuses the SAME discovery detector (definition + KeyPool/parser) as the Android probe
so the two tracks apply the identical recall bar.

Resumable: skips review_ids already in the verdicts file.

Run:
  python -m ios.phase-4-layer1-broad.recover_discovery_ios [--piles ux,updates] [--batch 10]
Outputs:
  data/interim/phase4_recover_verdicts_ios.jsonl  — every probed review + verdict (resume log)
  data/interim/ios_discovery_recovered.jsonl      — the recovered discovery reviews
  data/interim/ios_discovery_pool.jsonl           — FINAL iOS Phase-5 pool (orig + recovered, deduped)
  data/interim/ios_recover_summary.json           — per-pile census miss rates
"""
...
log = get_logger("ios.phase4.recover")
load_dotenv(dotenv_path=str(C.ROOT / ".env"))

# Reuse the Android probe's detector (same discovery definition the 8B was given).
_spec = importlib.util.spec_from_file_location(
    "recall_probe", str(C.PHASES_DIR / "phase-4-layer1-broad" / "recall_probe.py"))
rp = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(rp)

LAYER1 = C.INTERIM_DIR / "ios_layer1.jsonl"


def build_pool(piles):
    rows = list(read_jsonl(LAYER1))
    out = []
    for r in rows:
...
def run(piles, batch_size):
    ...
    if not LAYER1.exists():
        raise SystemExit(f"Missing {LAYER1} — run iOS Layer-1 (classify.py ios) first.")
    pool_reviews = build_pool(piles)
    verdicts_path = C.INTERIM_DIR / "phase4_recover_verdicts_ios.jsonl"
    ...
    log.info("iOS recovery over piles=%s: %d reviews (%d done), batch=%d",
             piles, len(todo), len(done), batch_size)

    pool = rp.KeyPool(keys)   # defaults to C.GROQ_MODEL_DEEP (120B)
    t0 = time.time(); n = 0; recovered = 0; stopped = False
    ...
    try:
        for s in range(0, len(todo), batch_size):
            ...
            try:
                ...
                log.warning("  ALL 120B keys exhausted at %d probed — stopping; resume when budget recovers", n)
                stopped = True
                break
            if parsed is None:
    ...
    # Build outputs from the full verdicts log (idempotent even if stopped mid-run).
    ...
    rec_path = C.INTERIM_DIR / "ios_discovery_recovered.jsonl"
    ...
    # FINAL iOS Phase-5 pool: original discovery + recovered, deduped by id
    orig = [r for r in read_jsonl(LAYER1) if "discovery" in (r.get("categories") or [])]
    pool_path = C.INTERIM_DIR / "ios_discovery_pool.jsonl"
    seen = set(); n_pool = 0
    ...
    summary = {"track": "ios", "piles": piles, "probed": len(all_v), "recovered": len(hits),
               "stopped_exhausted": stopped,
    ...
    write_json(C.INTERIM_DIR / "ios_recover_summary.json", summary)
    log.info("%s %.0fs tokens=%d", "STOPPED (resume to finish)" if stopped else "DONE",
             time.time() - t0, sum(pool.tokens))
    print("\n========== iOS DISCOVERY RECOVERY (census of ux+updates) ==========")
    for p in piles:
    ...
    print(f"  iOS Phase-5 pool: {len(orig)} (layer1) + {n_pool-len(orig)} (recovered) = {n_pool}")
    if stopped:
        print("\n  NOTE: 120B budget exhausted mid-run; re-run to finish the remaining piles.")
